// Cut Range Node: trims data within a specified range
export const def = {
  type: 'cutrange',
  title: 'Cut Range',
...
export function process(getInput, params, ctx) {
  ...
  if ((params.mode || '裁剪') === '裁剪') return { out: [...inp.slice(0, startIdx), ...inp.slice(endIdx)] };
  return { out: inp.map((v, i) => (i >= startIdx && i < endIdx) ? 0 : v) };
}
